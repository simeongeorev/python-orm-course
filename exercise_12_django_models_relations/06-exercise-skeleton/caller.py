...
def show_all_authors_with_their_books() -> str:
    ...
    for a in all_authors:
        books = a.book_set.all()  # all books for author
...
def calculate_licenses_expiration_dates():
    # 1. Grab ONLY the required fields from the DB as light tuples
    licenses = (DrivingLicense.objects.order_by("-license_number")
                .values_list('license_number', 'issue_date'))

    # 2. Loop over the raw data instead of heavy model instances
    result = [
        f"License with number: {l_number} expires on {i_date + timedelta(days=365)}!"
        for l_number, i_date in licenses
    ]
...
def get_drivers_with_expired_licenses(due_date: date):
    ...
    for d in drivers:
        ...
        if due_date > exp_date:
            result.append(d)

    return result

    # # FROM AI:
    # # Calculate the cutoff issue date in Python
    # cutoff_date = due_date - timedelta(days=365)
    #
    # # Filter directly in the database using the relationship spanning '__'
    # return Driver.objects.filter(license__issue_date__gt=cutoff_date)
