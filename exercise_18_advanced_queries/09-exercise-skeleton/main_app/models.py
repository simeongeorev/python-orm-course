...
class VideoGame(models.Model):
    RATING_MESSAGE = "The rating must be between 0.0 and 10.0"
    RELEASE_YEAR_MESSAGE = "The release year must be between 1990 and 2023"
    GENRE_CHOICES = [
        ('Action', 'Action'),
        ('RPG', 'RPG'),
        ('Adventure', 'Adventure'),
    ...
    release_year = models.PositiveIntegerField(validators=[
        MinValueValidator(1990, message=RELEASE_YEAR_MESSAGE),
        MaxValueValidator(2023, message=RELEASE_YEAR_MESSAGE)
    ])
    rating = models.DecimalField(max_digits=2,
                                 decimal_places=1,
                                 validators=[
                                     MinValueValidator(0, message=RATING_MESSAGE),
                                     MaxValueValidator(10.0, message=RATING_MESSAGE)
                                 ])
...
class Exercise(models.Model):
    ...
    def get_exercises_within_duration(cls, min_duration: int, max_duration: int):
        return cls.objects.filter(
            Q(duration_minutes__gte=min_duration) &
